...
<script setup lang="ts">
  ...
  import { fetchGetUserPermissionGroups, fetchUpdateUserPermissionGroups } from '@/api/system/user'
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // 获取用户已绑定的权限组
      const userGroups = await fetchGetUserPermissionGroups(props.userId)
      boundGroupIds.value = (userGroups || []).map(g => g.id)
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetchUpdateUserPermissionGroups(props.userId, boundGroupIds.value)
