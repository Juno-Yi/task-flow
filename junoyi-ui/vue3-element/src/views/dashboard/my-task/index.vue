        ...
        <template #default>
          <div v-if="currentTaskDetail" class="space-y-4">
            ...
            <ElDescriptions :column="2" border>
              ...
              <ElDescriptionsItem label="计划时间" :span="2">{{ formatPlanPeriod(currentTaskDetail.planStartTime, currentTaskDetail.planEndTime) }}</ElDescriptionsItem>
              <ElDescriptionsItem label="预计工时">{{ calculateHours(currentTaskDetail.planStartTime, currentTaskDetail.planEndTime) }}</ElDescriptionsItem>
              <ElDescriptionsItem label="是否逾期">
                <ElTag :type="currentTaskDetail.isOverdue ? 'danger' : 'success'" size="small">
...
const formatPlanPeriod = (startTime?: string, endTime?: string) => {
  if (startTime && endTime) {
    ...
    return `${formatCompactTime(startTime)} ~ 未设置`
  } else if (endTime) {
    return `未设置 ~ ${formatCompactTime(endTime)}`
  }
  return '未设置'
}
